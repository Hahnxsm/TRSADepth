...
from .common import _init_weights,act_layer,channel_shuffle,gcd
...
class MSSAM(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes=[1, 3, 5], expansion_factor=6, dw_parallel=True,
                 activation='elu', add=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.cab = CAB(in_channels, in_channels)
        self.sab = SAB()
        self.mscb = MSCBLayer(in_channels, out_channels, n=1, stride=1, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add, activation=activation)
