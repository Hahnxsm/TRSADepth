...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        elif opt.dataset == "kitti_depth":
            dataset = datasets.KITTIDepthDataset(opt.data_path, filenames,
                                               encoder_dict['height'], encoder_dict['width'],
                                               [0], 4, is_train=False)
        elif opt.dataset == "midair":
            dataset = datasets.MidAirDataset(opt.data_path, filenames,
                                               encoder_dict['height'], encoder_dict['width'],
                                               [0], 4, is_train=False)
        elif opt.dataset == "uavid":
            dataset = datasets.UAVidDepthDataset(opt.data_path, filenames,
                                               encoder_dict['height'], encoder_dict['width'],
                                               [0], 4, is_train=False)
        elif opt.dataset == "uav_ula":
            dataset = datasets.UAVulaDataset(opt.data_path, filenames,
                                               encoder_dict['height'], encoder_dict['width'],
                                               [0], 4, is_train=False)
        elif opt.dataset == "henu":
            dataset = datasets.HENUDataset(opt.data_path, filenames,
                                             encoder_dict['height'], encoder_dict['width'],
                                             [0], 4, is_train=False)
        else:
        ...
        encoder = efficientvit_backbone_l3()
        ...
        missing_keys, unexpected_keys =  depth_decoder.load_state_dict(torch.load(decoder_path),strict=False)
        print("Missing keys:", missing_keys)
        print("Unexpected keys:", unexpected_keys)
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                depth_gt.append(data["depth_gt"][:,0].detach())
    ...
    for i in range(pred_disps.shape[0]):
        ...
        if opt.export_pics:
            save_maps(gt_depth_save, pred_depth_save*ratio, i)
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print(f"Evaluate total time: {end_time - start_time:.2f} seconds")
