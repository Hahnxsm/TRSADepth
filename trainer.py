...
from networks.depth import efficientvit_backbone_l3
from utils import *

PROJECT = "TRSA"
experiment_name="UAV_ula"
...
class Trainer:
    def __init__(self, options):
        ...
        self.models['encoder'] = efficientvit_backbone_l3()
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "resnet":
                self.models["pose"] = pose.PoseNet()
            else:
        ...
        self.param_groups = [{"params":# list(self.models["encoder"].parameters())+
                                   list(self.models["depth"].parameters()) +
                                   list(self.models["pose"].parameters()), "lr": 1e-4},
                             {"params": list(self.models["encoder"].parameters()), "lr": 1e-5}
                             ]
        if self.opt.pose_model_type == "separate_resnet":
            self.param_groups += [{"params":list(self.models["pose_encoder"].parameters()), "lr": 1e-4},]
        # 初始使用 AdamW
        self.adam_optimizer = optim.AdamW(self.param_groups, weight_decay=0.01, betas=(0.9, 0.999))

        # 默认使用 AdamW
        self.model_optimizer = self.adam_optimizer
        ...
        datasets_dict = {"kitti": datasets.KITTIRAWDataset,
                         ...
                         "uav_ula":datasets.UAVulaDataset,
                         'uavid':datasets.UAVidDataset}
        ...
        self.model_lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.model_optimizer,T_max=self.num_total_steps,eta_min=1e-7)
        ...
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    elif self.opt.pose_model_type == "resnet":
    ...
    def val(self, num_batches=10):
        """Validate the model on multiple minibatches and log average loss"""
        self.set_eval()
        total_losses = {}
...
def visualize_depth(depth, cmap=cv2.COLORMAP_JET):
    ...
    """
    x = depth.detach().cpu().numpy()  # 将深度图放到cpu并转换为np数组
